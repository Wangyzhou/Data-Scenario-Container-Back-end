...
import sceneContainer_backend.pojo.dto.TileVectorLayersDTO;
...
// lines_pg   gis_osm_transport_free_1
    ...
    public static String envelopeToSQLMulti(List<TileVectorLayersDTO> layersInfoList, HashMap<String,Double> env){
        String envSql=envelopeToBoundsSQL(env);
        String multiMvtSql="select( ";
        for(int i=0;i<layersInfoList.size();i++){
            String shpTableName=layersInfoList.get(i).getId();
...
//        HashMap<String,String> table=new HashMap<String,String>();
////        table.put("table",shpName);
////        table.put("srid","4326");
////        table.put("geomColumn","geom");
////        table.put("attrColumns"," * ");
////        table.put("env",envelopeToBoundsSQL(env));
//
//        String mvtsql= MessageFormat.format("select("+
//                    "(WITH" +
//                            " bounds AS ( SELECT {0} AS geom, {0}::box2d AS b2d)," +
//                            " mvtgeom AS (" +
//                            " SELECT ST_AsMVTGeom(ST_Transform(t.{1}, 3857), bounds.b2d) AS geom, {2}" +
...
//                            " WHERE ST_Intersects(t.{1}, ST_Transform(bounds.geom, {4}))" +
//                            " )" +
//                            " SELECT ST_AsMVT(mvtgeom.* , ''my22'' ) FROM mvtgeom)"+
//                        "||"+
//                            "(WITH" +
//                            " bounds AS ( SELECT {0} AS geom, {0}::box2d AS b2d)," +
//                            " mvtgeom AS (" +
...
//                            " WHERE ST_Intersects(t.{1}, ST_Transform(bounds.geom, {4}))" +
//                            " )" +
//                            " SELECT ST_AsMVT(mvtgeom.* , ''my2'' ) FROM mvtgeom)"+
//                        ")as st_asmvt",
//                table.get("env"),table.get("geomColumn"),table.get("attrColumns"),table.get("table"),table.get("srid"));
