    ...
    public Boolean shp2pgsql(String shpFileName, String tableName, String espg, String code) {
        try {
            ...
            if (System.getProperty("os.name").toLowerCase().contains("windows")) {
                String cmd = MessageFormat.format(shp2pgSqlWin, postgresqlBinDir, espg, code, shpFileName, tableName);
                log.info(cmd);
                pro = Runtime.getRuntime().exec(cmd, new String[]{"PGPASSWORD=" + pgPassword});
            } else {
                String cmd = MessageFormat.format(shp2pgSqlLinux, postgresqlBinDir, espg, shpFileName, tableName);
                log.info(cmd);
...
//                re.get(i).forEach(map -> singleRecord.put(map));
    ...
    public ResponseResult getUniqueValues(String ptName, String field, String method) {
        ...
        return new ResponseResult(200, "成功", uniqueValues);
    }
}
